...
$REIMPRESO = '';
if($row['impreso']!=''){
	$REIMPRESO = "**********TICKET de DEVOLUCION**********";
}else{
	$result = $consulta->query("UPDATE ventas SET impreso='".date("Y-m-d H:i:s")."' WHERE idventas=".$_GET["idventas"]." ");
...
<div id="resultados_ticket" style="position:absolute;top:0px;left:0px;margin:0px;padding:0px;width:<?= $ancho ?>px; height:<?= (60) + 230 + $altu ?>px;border:0px solid; font:Arial, Helvetica, sans-serif; font-family:Arial, Helvetica, sans-serif;">
   ...
   <div style="position:RELATIVE;left:5px;font-size:12px;width:<?= $ancho ?>px;text-align:left;">
      ...
      if($row['tipo']=='DEVUELTO'){
			echo "<center><b>Productos Devueltos</b></center><br>";
		}
		if($row['tipo']=='apartado'){
			echo "<center><b>Apartado</b></center><br>";
		}
		?>
   </div>
   <div style="position:RELATIVE;left:0px;width:<?= $ancho ?>px;text-align:left;">
         ...
         <div style="position:relative; width:<?= $ancho + 1 ?>px; height:30px;">
            <table width="100%" style="font-size:12px;" border="0" cellpadding="0" cellspacing="0">
               <tr style=" color:#000; font-weight:bold;;">
                  ...
                  <td>Descripcion</td>
                  <?
                       $resulta = $consulta->query("SELECT * FROM ventas WHERE idventas=".$IDVENTAS);
                       foreach ($resulta as $reg);
                       if ($reg['tipo']=="DEVUELTO") {
                        ?>
      ...
      <table width="100%" style="font-size:12px;" border="1" cellpadding="0" cellspacing="0">
         <?
            ...
            if ($reg["ESTADO"=="DEVUELTO"]) {
               ...
               ?>
         <tr>
            <td align="center" colspan="3"><?= strtoupper(utf8_decode($reg["productos"])) ?></td>
         </tr>
         <tr>
            <?
            if ($reg["ESTADO"!="DEVUELTO"]) {
            ?>
            <td align="center" colspan="3"><?= strtoupper("**********" . $reg["ESTADO"] . "**********") ?></td>
            <?
            } 
            ?>
         </tr>
      ...
      <?
         if( $row["tipo"]=="credito"){
			...
			if($row["tipo"]=="contado"){
            $imp = $row["importe"];
            echo '<div style="text-align:left; width:100%; font-size:16px; text-align:right;"><b>Importe Venta&nbsp;&nbsp;<span style="float:right;"> $&nbsp;'.number_format($row["importe"],2,'.',',').'</span></b></div>';
            $Auto = $consulta->query("SELECT * FROM ventas_detalle WHERE idventas=".$IDVENTAS." AND ESTADO LIKE 'DEVUELTO'");
            foreach ($Auto as $row);
            $precio0 = $row['precio'];
            $cantidad0 = $row['devuelto'];
            $imp_devuelto = $precio0 * $cantidad0;  
            $query9 = $consulta->query("SELECT * FROM ventas WHERE idventas LIKE '" . $IDVENTAS . "'");
            foreach ($query9 as $t);

            echo '<div style="text-align:left; width:100%; font-size:16px; text-align:right;"><b>Total Devuelto &nbsp;&nbsp;&nbsp;<span style="float:right;"> $&nbsp;'.number_format($t['totaldevuelto'],2,'.',',').'</span></b></div>';
            $total_v = $imp-$t['totaldevuelto'];
            echo '<div style="text-align:left; width:100%; font-size:16px; text-align:right;"><b>Total Venta &nbsp;&nbsp;&nbsp;<span style="float:right;"> $&nbsp;'.number_format($total_v,2,'.',',').'</span></b></div>';
