...
include('inc/conectar.php');
	$Auto = $consulta->query("SELECT * FROM clases WHERE fechabaja IS NULL");
	foreach ($Auto as $row){
		?>
		<tr>
...
include('inc/conectar.php');
	$Auto = $consulta->query("UPDATE clases SET fechabaja='".date("Y-m-d H:i:s")."' WHERE idclases=".$_POST['idregistro']." ");
	foreach ($Auto as $Autocontador);
	exit();
}
if($_POST['funcion']=='Guardar'){
include('inc/conectar.php');
	$Auto = $consulta->query("INSERT INTO clases SET nombre='".$_POST['nombre']."', fechaalta='".date("Y-m-d H:i:s")."'");
	foreach ($Auto as $Autocontador);
exit();
}
if($_POST['funcion']=='Editar'){
include('inc/conectar.php');
	$Auto = $consulta->query("UPDATE clases SET nombre='".$_POST['nombre']."' WHERE idclases=".$_POST['idregistro']);
	foreach ($Auto as $Autocontador);
exit();
}
...
include('inc/conectar.php');
	if($_POST['tipo']!='Nuevo'){
		$Auto = $consulta->query("SELECT * FROM clases WHERE idclases=".$_POST['idregistro']."");
		foreach ($Auto as $row);
	}
	?>
    <div class="modal-body" >
      ...
      <div class="modal-footer">
		...
		if($_POST['tipo']!='Nuevo'){
?>
		  <button type="button" class="btn btn-primary" id="editar" registros="<?=$_POST['idregistro']?>"><i class="bi bi-pencil"></i> Modificar Clase</button>
		  <?
		}else{
			?>
		  <button type="button" class="btn btn-primary" id="guardar"><i class="bi bi-floppy"></i> Guardar Clase</button>
		  <?
		}
		?>
      </div>
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><i class="bi bi-cookie"></i>  Clases</h5>
        ...
        <div class="col-md-12">
            <div class="row">
                ...
                <div class="col-6 text-center ">
                    <h3><i class="bi bi-cookie"></i> Clases</h3>
                </div>
                <div class="col">
                    <button type="button" id="Nuevo" class="btn btn-info btn-sm" ><i class="bi bi-plus-circle"></i> Clase Nueva</button>
...
$(document).ready(function() {
	...
	$(document).on("click","#guardar",function(e) {
		...
		$.ajax({
			...
			async:false,
			success: function(msg){
				alertify.success("Clase Agredado Exitosamente ");
	...
	$(document).on("click","#editar",function(e) {
		...
		$.ajax({
			...
			success: function(msg){
				//console.log(msg);
				alertify.success("Clase Modificada Exitosamente ");
				window.location="<?=$_SERVER["PHP_SELF"]?>";
			}
		});
    });
	...
	$(document).on("click",".eliminar",function(){
		var idregistro = $(this).attr("registros");
		alertify.confirm("Eliminacion",'¿Estas Seguro de Eliminar la Clase?', function(){
			$.ajax({
				type: "POST",
				url: "<?=$_SERVER["PHP_SELF"]?>",
				data: ({
				...
				async:false,
				success: function(msg){
					alertify.success("Clase eliminada Exitosamente "+msg);
					window.location="<?=$_SERVER["PHP_SELF"]?>";
