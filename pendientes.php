...
?>
 <div class="col-2 text-center margen border rounded carta" >
    <div class="row">
        <div class="col-12 border rounded border-primary bg-primary bg-gradient-secondary text-white">
            Buscar
        </div>
    </div>    
    <div class="row" >
        <div class="col-12 nombre">
            <select type="text" class="form-control form-control-sm" style="margin-top:3px;" id="idclases">
                ...
                <?php
                    ...
                    foreach ($Autom as $mem){
                        ?>
                        <option value="<?=$mem['idclases']?>" <? if($mem['idclases']==$_POST['idclases']) echo "selected" ?>><?=$mem['nombre']?></option>
                        <?
                    }
                ?>
            </select>
            <input list="datosClientes" autocomplete="off" name="" class="form-control" id="nombre" placeholder="Buscar Cliente" autocomplete value="<?=$nombreb?>">
            <datalist id="datosClientes" class="col-md-9" active style="width:1000px;">
            </datalist>
            <button class="btn btn-success btn-md" style="margin-top:3px;" id="buscar"><i class="bi bi-search"></i> Buscar</button>
            <select type="text" class="form-control form-control-sm" style="margin-top:3px;" id="estado" style="margin-top:5px;">
                <option value="0">Todas Estados</option>
                <option value="activa" <? if($_POST['estado']=='activa'){ echo "selected";}?>>Membresia Activa</option>
                <option value="vencida" <? if($_POST['estado']=='vencida'){ echo "selected";}?>>Membresia Vencida o Sin Activar</option>
            </select>
...
$(document).ready(function() {
    Cargar_Membresias("");
    function Cargar_Membresias(nombre){
        var estado = "0";
        if($("#estado option:selected").val()!='undefined')estado = $("#estado option:selected").val();
        var idclases = 0;
        if($("#idclases option:selected").val()!='undefined')idclases = $("#idclases option:selected").val();
        $.ajax({
			type: "POST",
			url: "<?=$_SERVER["PHP_SELF"]?>",
			data: ({
				funcion : "Cargar_Membresias",
                estado : estado,
                idclases: idclases,
                nombre: nombre
			}),
            ...
            error : function(xhr, status) {
                alertify.success('Disculpe, existió un problema');
            }
		});
    }
