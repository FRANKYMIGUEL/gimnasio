...
if($_POST['funcion']=='Carga_Ventas'){
	...
	<?
	$resultados=$consulta->query("SELECT * FROM ventas WHERE fecha BETWEEN '".$_POST['fechai']." 00:00:00' AND '".$_POST['fechaf']." 23:00:00' ORDER BY ventas.idventas DESC");
...
<?
	...
	?>
        <tfoot>
            <tr>
                <td colspan="2">&nbsp;</td>
                <td align="right">Total Ventas $ <?=number_format($IMPORTE,2)?></td>
            </tr>
        </tfoot>
    </table>
    <?
...
$(document).ready(function(e) {
	...
	$(document).on("change","#estado, #fechainicial, #fechafinal, #idcliente, #idusuarios",function(){
		var ruta = '<?=$_SERVER["REQUEST_URI"];?>';
		ruta = ruta.split("?");
		if(ruta[1]== undefined){
			window.location = "reporte_ventas.php?fechainicial="+$("#fechainicial").val()+"&fechafinal="+$("#fechafinal").val();
		}else{
			window.location = "reporte_ventas.php?fechainicial="+$("#fechainicial").val()+"&fechafinal="+$("#fechafinal").val();
		}
	});
