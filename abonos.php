...
include("inc/conectar.php");
    ...
    <div class="row">
         ...
         <div class="col-lg-3">
            <button class="btn btn-info col-lg-12" type="button">
              Saldo <span class="badge">$ <?=number_format($row['saldo'],2)?></span>
            </button>
	...
	?>
         <div class="col-lg-3">
            <div class="progress">
              <div class="progress-bar progress-bar-success progress-bar-striped" role="progressbar" aria-valuenow="<?=$porcentaje?>" aria-valuemin="0" aria-valuemax="100" style="width: <?=number_format($porcentaje,2)?>%">
                <?=number_format($porcentaje,2)?>% de Pagado
              </div>
            </div>
         </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-lg-4 text-center">
           <div class="row">
               <b>Importe </b><input type="text" value="0" id="importe_abono" class="form-control text-right" autocomplete="off" />
           </div>
           <div class="row">
...
include("inc/conectar.php");
    ...
    <table class="table">
    <?
	$Auto = $consulta->query("SELECT * FROM ventas WHERE idclientes=".$clientes['idclientes']." AND fechapago IS NULL AND tipo NOT LIKE 'Cancelada' ");
	foreach ($Auto as $fila){
		if($fila['abonos']<$fila['importe']){
		?>
			<tr>
            	<td><?=$fila['folio']?></td>
            	<td><?=$fila['fecha']?></td>
            	...
            	<td align="right">$ <?=number_format($fila['importe']-$fila['abonos'],2)?></td>
            	<td width="100">
				 ...
				 <td width="50" class="text-right" title="Abonar" class="abonar">
					<a href="ticket_venta.php?idventas=<?=$fila['idventas']?>" target="_blank"><button type="button" class="btn btn-success">Imprimir</button></a>
				</td>
            </tr>
		<?
		}
        ...
        <div class="col-md-12">
            <div class="row ">
                <div class="col-12">
                    ...
                    <input list="datosClientes" name="" value="" class="form-control" id="clientes" placeholder="Buscar clientes">
                    <datalist id="datosClientes" active>
                    </datalist>
                    </div>
                    <div class="form-group col-md-2 ">
                    <br>
                     <button type="button" class="btn btn-block btn-md btn-success" id="buscar"><span class="glyphicon glyphicon-print" aria-hidden="true"></span> Buscar </button>
                    </div>
                    <div class="col-md-5">
            ...
            <div class="row ">
                <div class="col-12">
                    <div class="row ">
                        <div class="col-sm-12 ">
                            <table class="table table-hover table-condensed" width="90%">
                                <thead>
                                    <tr style="background-color:#2e353d; color:#FFF;">
                                        <td width="100"  class="text-center">Folio</td>
                                        <td class="text-center">Fecha</td>
                                        <td width="150" class="text-center">Importe</td>
